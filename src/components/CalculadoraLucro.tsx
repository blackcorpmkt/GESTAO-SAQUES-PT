...
type MoedaAds = 'EUR' | 'BRL' | 'USD'
interface CustoExtra { id: string; descricao: string; valor: string }
...
export function CalculadoraLucro({ config, onToast }: Props) {
  ...
  const [moedaAds, setMoedaAds] = useState<MoedaAds>('EUR')
  const [cotacaoAds, setCotacaoAds] = useState('')
  const [custos, setCustos] = useState<CustoExtra[]>([{ id: crypto.randomUUID(), descricao: '', valor: '' }])
  ...
  const addCusto = () => setCustos(prev => [...prev, { id: crypto.randomUUID(), descricao: '', valor: '' }])
  const removeCusto = (id: string) => setCustos(prev => prev.filter(c => c.id !== id))
  const updateCusto = (id: string, campo: 'descricao' | 'valor', valor: string) =>
  ...
  // Gasto em anúncios convertido para EUR conforme a moeda selecionada
  const anunciosRaw = num(gastoAnuncios)
  const taxaAds = num(cotacaoAds)
  const anunciosEur =
    moedaAds === 'EUR'
      ? anunciosRaw
      : moedaAds === 'BRL'
        ? (taxaAds > 0 ? anunciosRaw / taxaAds : 0)
        : anunciosRaw * taxaAds // USD: valor × cotação (€ por US$)

  const totalOutros = custos.reduce((s, c) => s + num(c.valor), 0)

  const liquido = calcularLiquido(bruto, pctGateway, fixaGateway, vendas)
  const custoGateway = bruto - liquido
  const totalCustos = custoGateway + anunciosEur + totalOutros
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <div className="flex gap-2">
              ...
              <select
                value={moedaAds}
                onChange={e => setMoedaAds(e.target.value as MoedaAds)}
                className="border border-gray-200 dark:border-gray-600 rounded-xl px-2 py-2.5 text-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
              >
            ...
            {moedaAds !== 'EUR' && (
              <input
                type="text" inputMode="decimal"
                value={cotacaoAds}
                onChange={e => setCotacaoAds(e.target.value)}
                placeholder={moedaAds === 'BRL' ? 'Cotação: BRL por € (ex: 5.83)' : 'Cotação: € por US$ (ex: 0.92)'}
                className={`${inputClass} mt-2`}
              />
            )}
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 tabular-nums">
              ≈ {formatarEUR(anunciosEur)} em euros
              {moedaAds !== 'EUR' && taxaAds <= 0 && anunciosRaw > 0 ? ' — informe a cotação' : ''}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
            ...
            <span className="font-semibold text-gray-600 dark:text-gray-300 tabular-nums">{formatarEUR(totalOutros)}</span>
          </p>
        </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-800 rounded-xl p-3 text-xs text-blue-700 dark:text-blue-300 mt-5 font-mono">
          Lucro = Bruto − (Bruto × {pctGateway}%) − (€{fixaGateway} × vendas) − anúncios − outros custos
        </div>
      </div>
